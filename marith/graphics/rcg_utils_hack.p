...
/* --- Copyright University of Sussex 1991. All rights reserved. ----------
 > File:            C.all/x/pop/lib/rcg_utils.p
...
/* The next function should be part of RC_SET_SCALE rather than
in here, but here is where it lives for now.
*/

define rc_set_coord_frame(type, screen_region, user_region);
    ;;; Set the coordinate frame to map the user_region onto the
    ;;; screen_region. The screen region is specified relative to
    ;;; axes with their origin at the top left of the window, x going
    ;;; right and y going down the screen.  The units are given by
    ;;; the type argument, which is as in rc_set_scale. The region
    ;;; may be given either as a list of 4 numbers, xmin, xmax, ymin, ymax
    ;;; or as 4 separate arguments in the same order.
    ;;; The user region is specified the same way, using the values
    ;;; that will be defined for the region in the new coordinate frame.
    lvars type screen_region user_region;
    lvars sx0 sx1 sy0 sy1 ux0 ux1 uy0 uy1 xrat yrat;
    if user_region.isreal then
        type, screen_region, user_region
            -> (type, screen_region, ux0, ux1, uy0, uy1)
    else
        explode(user_region) -> (ux0, ux1, uy0, uy1)
    endif;
    if screen_region.isreal then
        type, screen_region
            -> (type, sx0, sx1, sy0, sy1)
    else
        explode(screen_region) -> (sx0, sx1, sy0, sy1)
    endif;
...
/* Should really be in RC_GRAPHIC, but here for now */
...
unless val = 1 and xory /= "y" and not(zero_one) then /*HACK*/

    unless val.isstring then nullstring >< val -> val endunless;
    if xory == "y" then
    ;;;    t - (length(val)+1) * rcg_char_width / rc_xscale -> t;
        t - (length(val)+1) * 12 / rc_xscale -> t;
    u + 0.3 * rcg_char_height / rc_yscale -> u; ;;; HACK
        rc_print_at(t,u,val)
    elseif xory == "x" then
        u - 0.5 * length(val) * rcg_char_width / rc_xscale -> u;
;;;        t + rcg_char_height / rc_yscale -> t; ;;; HACK
        t + 25 / rc_yscale -> t;
        rc_print_at(u,t,val)
    endif;
endunless;
...
define rcg_ladder(xory,tstart,tend,tinc,umin,umax);
    ...
    lvars t;
    lvars tstart tend tinc umin umax;
...
define rcg_def_ax(xmin,xmax,xlabl,ymin,ymax,ylabl);
    ...
    lvars xsize = xmax-xmin, ysize = ymax-ymin,
        ...
        xmarkinc = rcg_roundup(xsize/rcg_mk_no),
        xtickinc = xmarkinc/rcg_tk_no,
        ymarkinc = rcg_roundup(ysize/rcg_mk_no),
        ytickinc = ymarkinc/rcg_tk_no;
...
1 -> xmin; ;;; cahneg back to 2 if needed
endif;
